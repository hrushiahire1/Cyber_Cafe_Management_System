...
Computer::Computer(time_t _start_time = NULL, time_t _end_time = NULL) : start_time(_start_time), end_time(_end_time) {}//Paramterized Constructor using initializer list



void Computer::login_comp(time_t _start_time) { start_time = _start_time; } //Function used to set start time for the customer

void Computer::logout_comp(time_t _end_time, int* accessTime) { //Function used to find total session time for the customer after logging out
